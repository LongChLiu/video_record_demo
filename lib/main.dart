...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Video Player and Recorder'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _onVideoBtnPressed(ImageSource source) async {
    ...
    if(source == ImageSource.camera){
      PickedFile? recordedFile = await _picker.getVideo(source: ImageSource.camera, maxDuration: const Duration(seconds: 10));
      file = File(recordedFile!.path);
    }else{
      FilePickerResult? result = await FilePicker.platform.pickFiles(type: FileType.video);
      file = result!.files[0] as File;
    }
